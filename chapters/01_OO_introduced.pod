...
rather like Perl but without all the syntactic rigour.> a structure that represents a person:
...
Now all we have to do is add a row to the despatch table for every new object. That's a lot less pain. 
...
called B<methods>. So let's go ahead and expand the definition of C<Cat> a bit:
...
to create a new I<instance> of a I<class>, like this:
    ...
    my $tom = $Cat->{_methods}->{new}->( name => "Thomas O'Malley" );
...
Hopefully, you're starting to get the idea. However, this still isn't perfect. Among other things, the 
syntax is clunky, and the methods on our classes are all anonymous subroutines, which is going to 
make things like debugging hard. 
    
=head2 Proper OO

We've basically done all the above as if Perl had no OO features at all. 
