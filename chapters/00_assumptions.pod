...
If you are a desperate Perl programmer who's apparently restricted to the system Perl installation on a Unix machine, don't worry. Fortunately, getting a local Perl that you have full control over is a lot easier than it used to be. If you're on a Unix based OS, including any Linux distro´or MacOS X), I<perlbrew> comes to the rescuse, as long as you have access to a C compiler. Equally, on Windows, you can install Strawberry Perl and get a working Perl environment you control. Appendix 1 covers how to get going on both these platforms in much more detail if you need it.

=head3 Moose and its supporting modules

Once you have a working Perl that you control, you'll need to install Moose. The best way of doing this, which also picks up a bunch of other handy modules, is to install I<Task::Kensho::OOP> from the Enlightened Perl Organization's I<Task::Kensho> series. Appendix 2 will cover the details of how to do this for both Unix-like OS's and Windows, if you're unfamiliar with the process.

=head2 And now...

That's pretty much it. If you're comfortable with all the prerequisites, and think this is the book for you, let's get on with it!
